...
export class HomeComponent implements OnInit {
  ...
  setThecanvas(): any {
    const canvas: any | null = document.querySelector('.webgl');
    ...
    fontLoader.load(
      'assets/fonts/Fascinate_Regular.json',
      (font) => {
        ...
        for(let i = 0; i < 350; i++){
          ...
          meshBox.position.x = (Math.random() - 0.5) * 40;
          meshBox.position.y = (Math.random() - 0.5) * 40;
          meshBox.position.z = (Math.random() - 0.5) * 40;
    ...
    renderer.setClearColor( 0xe131212, 1 );
